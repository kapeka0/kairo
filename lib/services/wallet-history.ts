...
export async function computeWalletDayMap(
  ...
  switch (wallet.tokenType as TokenType) {
    case TokenType.BTC: {
      ...
      const entries = await btcBlockbook.fetchBalanceHistory(descriptor);
    ...
    case TokenType.ETH: {
      const { decimals } = getTokenMetadata(TokenType.ETH);
      const entries = await ethBlockbook.fetchBalanceHistory(wallet.publicKey);
      ...
      return { dayMap, tokenType: TokenType.ETH, decimals };
    }
    default:
